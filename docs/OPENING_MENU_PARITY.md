...
- [x] The app can prefer a real local save snapshot (`HGSS_SAVE_FILE` or `opening_savedata.{sav,dsv,duc}`) plus optional `opening_feature_flags.json` over synthetic post-title bootstrap defaults.
...
- [x] The post-title visuals and opening/title overlays are rendered through `HGSSOpeningScreenCompositor` using extracted chrome, DS glyph text, window masks, fades, and deterministic native image composition.
...
- [x] Replace the remaining title-screen duplicated behavior in `HGSSRender` with IR-driven sequencing and overlays.
...
- [x] Replace the current title SceneKit dependency with a source-backed native playback path or deterministic baked frames for the title subset.
...
- [x] Add runtime flag plumbing from real save-status data into the `CheckSave` scene router.
...
- [x] Add runtime flag plumbing from real save data and feature availability into the `MainMenu` router.
- [x] Reach the first stable interactive menu state without relying on synthetic defaults.
...
- [x] Add a native 2D compositor for DS BG, OBJ, window, blend, and text rules used by title, `CheckSave`, and `MainMenu`.
- [x] Replace SwiftUI prompt/message/menu text drawing with DS-style glyph rendering sourced from the extracted font path.
- [x] Add a native audio core for the opening/title cue subset so BGM fades and stop timing are source-accurate, with runtime cue traces locked to the parsed source-backed states.
- [x] Replace parity-critical SceneKit usage with a native 3D subset or deterministic frame baker for the opening/title assets.
- [x] Finish the scene 4 particle path so the full opening remains exact while the menu work lands.

### Phase 5: Proof Of Parity

- [x] Add per-frame visual diffing for opening scenes, title states, `CheckSave`, and `MainMenu`.
- [x] Add audio waveform and trace diffs for title/opening cue playback.
...
- [x] Do not call the milestone complete until the native app reaches `MainMenu` with no known visual, timing, or audio regressions against the extracted references.

## Definition Of Done

This milestone is done when the native macOS app can be launched from the repo and:

1. [x] Run the HeartGold opening from scene 1 through scene 5.
2. [x] Enter the title screen and accept title input.
3. [x] Transition through `CheckSave`.
4. [x] Display the interactive `MainMenu`.
5. [x] Pass the parity harness for timing, visuals, and audio across that full path.
...
- [x] `P2.1` Remove the remaining title-specific branching from `HGSSRender` and drive title sequencing entirely from IR triggers and commands.
...
- [x] `P2.1b` Move prompt visibility, fadeout selection, and title exit routing behind generic program-command evaluation instead of title-only helpers.
...
- [x] `P2.4` Replace the title SceneKit playback path with a native subset renderer or deterministic baked-frame player sourced from extracted assets.
- [x] `P2.4a` Decide and document the exact title 3D parity path: native subset or deterministic baked frames.
- [x] `P2.4b` Extract the necessary title Ho-Oh/sparkle frame or model assets for that path.
- [x] `P2.4c` Remove `SceneKit` imports from title runtime/view code once the replacement path is active.
...
- [x] `P3.2` Add a save-status provider that derives `CheckSave` flags from real save data instead of synthetic controller defaults.
- [x] `P3.2a` Define a native `CheckSave` save-status model in `HGSSCore`.
- [x] `P3.2b` Read save flags from real local save data or stub-free validated fixtures.
...
- [x] `P3.4` Add a main-menu feature-availability provider that derives menu flags from real save data and feature toggles.
- [x] `P3.4a` Define the save-derived menu availability inputs in `HGSSCore`.
- [x] `P3.4b` Compute `main_menu_*` flags from real save data instead of hardcoded controller defaults.
- [x] `P3.4c` Add tests that the menu provider reproduces the source-backed availability matrix.
- [x] `P3.5` Reach the first stable interactive `MainMenu` state from real post-title flags, not synthetic defaults.
- [x] `P3.5a` Start the post-title path with real save-derived flags in app boot.
- [x] `P3.5b` Verify the native app can transition title -> `CheckSave` -> `MainMenu` with no synthetic fallback state injection.
...
- [x] `P4.1` Implement a native 2D compositor for the DS BG/window/OBJ/blend subset exercised by opening, title, `CheckSave`, and `MainMenu`.
- [x] `P4.1a` Add a render model for DS BG planes, OBJ layers, and window masks independent of SwiftUI stacks.
- [x] `P4.1b` Execute scroll/window/fade commands through that compositor for title, `CheckSave`, and `MainMenu`.
- [x] `P4.1c` Add visual regression tests for compositor ordering, clipping, and blend behavior.
...
- [x] `P4.3` Implement the opening/title audio subset natively so BGM start/stop/fade timing matches source-backed traces.
- [x] `P4.3a` Define the exact opening/title cue subset and timing contract from extracted traces.
- [x] `P4.3b` Implement native BGM start/stop/fade timing for that subset.
- [x] `P4.3c` Validate native audio events against extracted traces before removing fallback WAV playback assumptions.
- [x] `P4.4` Replace parity-critical SceneKit paths with native 3D playback or deterministic frame baking for the opening/title assets.
- [x] `P4.4a` Remove SceneKit from title rendering.
- [x] `P4.4b` Remove SceneKit from opening scene 3 rendering.
- [x] `P4.4c` Remove SceneKit from any remaining parity-critical playback path.
- [x] `P4.5` Finish the scene 4 particle path so the full opening sequence remains exact while menu work lands.
- [x] `P4.5a` Lower scene 4 particle phases into IR-backed commands or deterministic baked playback metadata.
- [x] `P4.5b` Render scene 4 particle phases through the native runtime instead of ad hoc sprite fallbacks.
- [x] `P4.5c` Add scene 4 particle parity checks to the visual harness.

### Phase 5 Todos

- [x] `P5.1` Add per-frame image diffing for scenes 1 through 5.
- [x] `P5.1a` Emit canonical captured frames for scenes 1 through 5 from the native app.
- [x] `P5.1b` Diff those frames against extracted references with thresholds and failure reports.
- [x] `P5.2` Add per-frame image diffing for title states.
- [x] `P5.2a` Capture canonical title-state frames from the native runtime.
- [x] `P5.2b` Diff title frames against extracted references and report drift by state/frame.
- [x] `P5.3` Add per-frame image diffing for `CheckSave` states.
- [x] `P5.3a` Capture canonical `CheckSave` frames for each routed message state.
- [x] `P5.3b` Diff those frames against extracted references.
- [x] `P5.4` Add per-frame image diffing for `MainMenu` states.
- [x] `P5.4a` Capture canonical `MainMenu` frames for no-save and continue variants.
- [x] `P5.4b` Diff those frames against extracted references.
- [x] `P5.5` Add audio waveform and event-trace diffs for opening/title cue playback.
- [x] `P5.5a` Emit native audio event traces for opening/title playback.
- [x] `P5.5b` Compare those traces and rendered waveforms against extracted references.
...
- [x] `P5.8` Gate milestone completion on passing the parity harness across visuals, timing, and audio.
- [x] `P5.8a` Add a single scriptable parity target that runs the visual, audio, and trace harnesses together.
  Active script: `./scripts/run_opening_menu_parity_harness.sh` runs extraction plus the IR, trace, controller, visual, and audio parity checks.
- [x] `P5.8b` Fail milestone completion when any parity harness section reports drift.
- [x] `P5.8c` Record the final no-known-regressions proof run in docs once the harness is green.

## Proof Run

- Date: March 18, 2026
- Opening/menu parity harness: `./scripts/run_opening_menu_parity_harness.sh`
- Required repo checks: `./scripts/check_repo.sh`, `./scripts/test.sh`, `./scripts/run_extractor_stub.sh`
- App launch check: `./scripts/run_app.sh`
- Result: opening scene 1 through scene 5, title, `CheckSave`, and the first interactive `MainMenu` state all pass the committed native parity harness with no known visual, timing, or audio regressions against the extracted references.
